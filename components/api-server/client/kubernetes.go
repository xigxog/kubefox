...
func (c *k8sClient) LookupPlatform(namespace string) (*kubev1a1.Platform, error) {
...
func (c *k8sClient) Create(kit kubefox.Kit, obj k.Object) error {
	kit.Log().Debugf("creating %s", kubernetes.FullKey(obj))
	gvk := obj.GetObjectKind().GroupVersionKind()

	err := c.kube.Create(kit, obj)
	obj.GetObjectKind().SetGroupVersionKind(gvk)
	return err
}

func (c *k8sClient) Put(kit kubefox.Kit, obj k.Object) error {
	kit.Log().Debugf("updating %s", kubernetes.FullKey(obj))

	gvk := obj.GetObjectKind().GroupVersionKind()
	if err := c.kube.Update(kit, obj); err != nil {
		obj.GetObjectKind().SetGroupVersionKind(gvk)
		if IsNotFound(err) {
...
func (c *k8sClient) Patch(kit kubefox.Kit, obj k.Object) error {
	kit.Log().Debugf("patching %s", kubernetes.FullKey(obj))
	gvk := obj.GetObjectKind().GroupVersionKind()

	if err := c.kube.Patch(kit, obj, k.Merge, c.patchOpts); err != nil {
		obj.GetObjectKind().SetGroupVersionKind(gvk)
		if IsNotFound(err) {
			kit.Log().Debugf("resource %s not found, creating it", kubernetes.FullKey(obj))
			return c.Create(kit, obj)
...
func (c *k8sClient) Get(kit kubefox.Kit, obj k.Object) error {
	kit.Log().Debugf("getting %s", kubernetes.FullKey(obj))
	gvk := obj.GetObjectKind().GroupVersionKind()

	err := c.kube.Get(kit, k.ObjectKeyFromObject(obj), obj)
	obj.GetObjectKind().SetGroupVersionKind(gvk)
	return err
}

func (c *k8sClient) List(kit kubefox.Kit, list k.ObjectList, labels map[string]string) error {
	return c.list(kit, list, "", labels)
}

func (c *k8sClient) ListNamespaced(kit kubefox.Kit, list k.ObjectList, sys string, labels map[string]string) error {
	return c.list(kit, list, sys, labels)
}

func (c *k8sClient) list(kit kubefox.Kit, list k.ObjectList, sys string, labels map[string]string) error {
	kit.Log().Debugf("listing %s", kubernetes.KindKey(list))
	gvk := list.GetObjectKind().GroupVersionKind()
	namespace := utils.SystemNamespace(kit.Platform(), sys)

	err := c.kube.List(kit, list, k.InNamespace(namespace), k.MatchingLabels(labels))
	list.GetObjectKind().SetGroupVersionKind(gvk)
	return err
}

func (c *k8sClient) Delete(kit kubefox.Kit, obj k.Object) error {
	kit.Log().Debugf("deleting %s", kubernetes.FullKey(obj))

	return c.kube.Delete(kit, obj)
}
